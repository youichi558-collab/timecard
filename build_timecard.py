...
# 時刻入力欄の表示形式。930(HHMM整数)を 09:30 と表示する。
#
# 以前は [<1]h:mm;00":"00 のように h:mm を含む条件付き書式にして、コロン付き
# 入力(時刻シリアル値)も同じ書式内で見やすく表示しようとしたが、実機Excelでは
# 書式コードに h:mm(日付/時刻トークン)が含まれると、セル全体が「日付/時刻型」
# として扱われてしまい、930 のような整数値が日付(1902年ごろ)として表示される
# 不具合が起きた。LibreOfficeでは再現せず実機Excelでのみ発生する差異だった。
#
# 日付/時刻トークンを一切含まない、純粋な数値の桁分割書式だけを使う。コロン
# 付き入力(1未満の小数)はこの書式では 00:00 に見えてしまうが、計算(集計)は
# to_serial() 側で正しく処理されるため、表示上の見た目だけの制約として許容する。
TIME_FMT = '00":"00'
