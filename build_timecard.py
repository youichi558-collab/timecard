...
# 時刻入力欄の表示形式。930(HHMM整数)は "00\":\"00" で 09:30 と表示する。
# 一方、9:30 とコロン付きで入力した場合、Excel はセルの書式を上書きせず
# 既存の書式のまま値だけ時刻シリアル値(1未満の小数)として取り込むため、
# 数値専用の書式だとコロン入力が 00:00 に見えてしまう。
# 1未満なら h:mm、そうでなければ数字分割の "00\":\"00" を使う条件付き書式にして
# どちらの入力方法でも正しく表示されるようにする。
TIME_FMT = '[<1]h:mm;00":"00'
